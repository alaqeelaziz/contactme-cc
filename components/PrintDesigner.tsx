...
export default function PrintDesigner({ title, subtitle, steps, downloadPdfLabel, downloadPngLabel, noteLabel, profileName, profileTitle, profilePhone, profileEmail }: Props) {
  ...
  // ── FRONT designs per size ──────────────────────────────────────────────────

  async function drawFrontStandard(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    const txtC = isDark ? '#FFF' : '#111827'
    const subC = theme === 'dark' ? '#93C5FD' : theme === 'gradient' ? 'rgba(255,255,255,0.85)' : colors.primary
    const metC = isDark ? 'rgba(255,255,255,0.65)' : '#6B7280'
    const dotC = theme === 'gradient' ? 'rgba(255,255,255,0.9)' : isDark ? '#FFF' : colors.primary
    const n = name || 'اسمك هنا'; const ini = n.charAt(0)
    ...
    if (logoUrl) {
      ...
      ctx.fillStyle = theme === 'gradient' ? 'rgba(255,255,255,0.25)' : (() => { const g = ctx.createLinearGradient(AX,AY,AX+AS,AY+AS); g.addColorStop(0,colors.primary); g.addColorStop(1,colors.secondary); return g })()
      ctx.fill(); ctx.fillStyle = '#FFF'; ctx.font = `bold ${Math.round(AS*.4)}px Tahoma,Arial`
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(ini, AX+AS/2, AY+AS/2)
    }
    // Name RTL
    ...
    ctx.fillStyle = subC; ctx.font = '12px Tahoma,Arial'; ctx.fillText(jobTitle || 'المسمى الوظيفي', AX - 10, AY + 22); ctx.restore()

    // Dot grid top-left
    const GX = ox + 16, GY = oy + 16, GS = 38
    ...
    // Divider
    const divY = oy + Math.round(H * .55)
    ctx.save(); ctx.globalAlpha = .18; ctx.fillStyle = txtC; ctx.fillRect(ox+16, divY, W-32, 1); ctx.restore()

    // Contact LTR
    ctx.save(); ctx.direction = 'ltr'; ctx.textAlign = 'left'; ctx.textBaseline = 'top'
    ctx.fillStyle = metC; ctx.font = '11px Arial,sans-serif'
    let cy = divY + 12
    if (phone) { ctx.fillText('📞  ' + phone, ox+16, cy); cy += 20 }
    if (email) { ctx.fillText('✉  '  + email, ox+16, cy); cy += 20 }
    ctx.font = '10px Arial,sans-serif'; ctx.fillText('🌐  ' + qr, ox+16, cy); ctx.restore()
  }
  ...
  async function drawFrontSquare(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    const g2 = ctx.createLinearGradient(ox, oy, ox+W, oy)
    g2.addColorStop(0, colors.primary); g2.addColorStop(1, colors.secondary)
    ctx.fillStyle = g2; ctx.fillRect(ox, oy, W, 5)

    // Center avatar
    const AS = 72, AX = ox + (W - AS) / 2, AY = oy + 28
    if (logoUrl) {
      const img = await loadImg(logoUrl)
      ctx.save(); ctx.beginPath(); ctx.roundRect(AX, AY, AS, AS, AS/2); ctx.clip()
      ctx.drawImage(img, AX, AY, AS, AS); ctx.restore()
    } else {
      ctx.beginPath(); ctx.roundRect(AX, AY, AS, AS, AS/2)
      const ag = ctx.createLinearGradient(AX, AY, AX+AS, AY+AS)
      ag.addColorStop(0, colors.primary); ag.addColorStop(1, colors.secondary); ctx.fillStyle = ag; ctx.fill()
      ctx.fillStyle = '#FFF'; ctx.font = `bold ${Math.round(AS*.38)}px Tahoma,Arial`
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(ini, AX+AS/2, AY+AS/2)
    }
    // Ring
    ctx.save(); ctx.beginPath(); ctx.roundRect(AX-2, AY-2, AS+4, AS+4, AS/2+2)
    ctx.strokeStyle = isDark ? 'rgba(255,255,255,0.4)' : colors.primary; ctx.lineWidth = 2; ctx.stroke(); ctx.restore()
    ...
    ctx.fillStyle = subC; ctx.font = '12px Tahoma,Arial'; ctx.fillText(jobTitle||'المسمى الوظيفي', ox+W/2, AY+AS+32); ctx.restore()

    // Divider
    const divY = AY + AS + 55
    ctx.save(); ctx.globalAlpha = .15; ctx.fillStyle = txtC; ctx.fillRect(ox+30, divY, W-60, 1); ctx.restore()

    // Contact centered LTR
    ctx.save(); ctx.direction = 'ltr'; ctx.textAlign = 'left'; ctx.textBaseline = 'top'
    ctx.fillStyle = metC; ctx.font = '11px Arial,sans-serif'
    let cy = divY + 12; const lx = ox + 30
    if (phone) { ctx.fillText('📞  ' + phone, lx, cy); cy += 20 }
    if (email) { ctx.fillText('✉  '  + email, lx, cy); cy += 20 }
    ctx.font = '10px Arial,sans-serif'; ctx.fillText('🌐  ' + qr, lx, cy); ctx.restore()
  }
  ...
  async function drawFrontPortrait(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    ctx.save()
    const hg = ctx.createLinearGradient(ox, oy, ox+W, oy+H*.4)
    hg.addColorStop(0, isDark ? 'rgba(255,255,255,0.07)' : `${colors.primary}18`); hg.addColorStop(1,'rgba(0,0,0,0)')
    ctx.fillStyle = hg; ctx.fillRect(ox, oy, W, H*.4); ctx.restore()

    // Accent top line
    const gl = ctx.createLinearGradient(ox, oy, ox+W, oy)
    gl.addColorStop(0, colors.primary); gl.addColorStop(1, colors.secondary)
    ctx.fillStyle = gl; ctx.fillRect(ox, oy, W, 4)
    ...
    ctx.save(); ctx.beginPath(); ctx.roundRect(AX-2, AY-2, AS+4, AS+4, AS/2+2)
    ctx.strokeStyle = isDark ? 'rgba(255,255,255,0.4)' : colors.primary; ctx.lineWidth = 2.5; ctx.stroke(); ctx.restore()

    // Name RTL centered
    ctx.save(); ctx.direction = 'rtl'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'
    ctx.fillStyle = txtC; ctx.font = 'bold 14px Tahoma,Arial'; ctx.fillText(n, ox+W/2, AY+AS+14)
    ctx.fillStyle = subC; ctx.font = '11px Tahoma,Arial'; ctx.fillText(jobTitle||'المسمى الوظيفي', ox+W/2, AY+AS+30); ctx.restore()

    // Divider with dot
    const lineY = AY+AS+52
    ctx.save(); ctx.globalAlpha = .2; ctx.fillStyle = txtC; ctx.fillRect(ox+16, lineY, W-32, 1); ctx.restore()
    ctx.save(); ctx.globalAlpha = .6; ctx.beginPath(); ctx.roundRect(ox+W/2-3, lineY-3, 6, 6, 3)
    ctx.fillStyle = isDark ? '#FFF' : colors.primary; ctx.fill(); ctx.restore()

    // Contact LTR
    ctx.save(); ctx.direction = 'ltr'; ctx.textAlign = 'left'; ctx.textBaseline = 'top'
    ctx.fillStyle = metC; ctx.font = '12px Arial,sans-serif'
    let cy = lineY + 14; const lx = ox+20
  ...
  async function drawFrontLarge(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    // Left color band
    ctx.save()
    const vg = ctx.createLinearGradient(ox, oy, ox, oy+H)
    vg.addColorStop(0, colors.primary); vg.addColorStop(1, colors.secondary)
    ctx.fillStyle = vg; ctx.fillRect(ox, oy, 80, H); ctx.restore()
    ...
    // Dot grid in band (decorative)
    const DX = ox+18, DY = oy+H-48
    for (let i = 0; i < 6; i++) {
      if ([2,5].includes(i)) continue
      ctx.beginPath(); ctx.roundRect(DX+(i%3)*13, DY+Math.floor(i/3)*13, 9, 9, 2)
    ...
    ctx.fillStyle = subC; ctx.font = '13px Tahoma,Arial'; ctx.fillText(jobTitle||'المسمى الوظيفي', ox+W-16, oy+46); ctx.restore()

    // Divider
    const divY = oy + Math.round(H*.42)
    ctx.save(); ctx.globalAlpha = .15; ctx.fillStyle = txtC; ctx.fillRect(RX, divY, W-RX-ox-16, 1); ctx.restore()

    // Contact LTR
    ctx.save(); ctx.direction = 'ltr'; ctx.textAlign = 'left'; ctx.textBaseline = 'top'
    ctx.fillStyle = metC; ctx.font = '12px Arial,sans-serif'
    let cy = divY + 16
    if (phone) { ctx.fillText('📞  ' + phone, RX, cy); cy += 26 }
    if (email) { ctx.fillText('✉  '  + email, RX, cy); cy += 26 }
    ctx.font = '11px Arial,sans-serif'; ctx.fillText('🌐  ' + qr, RX, cy); ctx.restore()
  }
  ...
  async function drawBack(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    const qs = Math.round(Math.min(W, H) * .48)
    const qx = ox + Math.round((W - qs) / 2), qy = oy + Math.round((H - qs) / 2) - 10
    const pad = 12
    ...
    ctx.save(); ctx.direction = 'ltr'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'
    ctx.fillStyle = metC; ctx.font = '9px Arial,sans-serif'
    ctx.fillText(qr, ox+W/2, qy+qs+pad+6); ctx.restore()
  }

  async function buildCanvas(): Promise<[HTMLCanvasElement, number, number]> {
    const PX = 3
    const { W, H, mmW, mmH } = size
    const isV = H > W // portrait orientation
    const GAP = 16

    // For portrait: stack vertically; else side by side
    const cvW = isV ? W * 2 + GAP : W * 2 + GAP
    const cvH = H
    const cv = document.createElement('canvas')
    cv.width = cvW * PX; cv.height = cvH * PX
    const ctx = cv.getContext('2d')!
    ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, cv.width, cv.height)
    ctx.scale(PX, PX)

    await drawFront(ctx, W, H, 0, 0)
    await drawBack(ctx, W, H, W + GAP, 0)

    // PDF mm for combined
    const pdfW = isV ? mmW * 2 + 4 : mmW * 2 + 4
    const pdfH = mmH
    return [cv, pdfW, pdfH]
  }
  ...
  async function handlePDF() {
    setLoading(true)
    try {
      const [cv, pdfW, pdfH] = await buildCanvas()
      const { jsPDF } = await import('jspdf')
      const orient = pdfH > pdfW ? 'portrait' as const : 'landscape' as const
      const pdf = new jsPDF({ orientation: orient, unit: 'mm', format: [pdfW, pdfH] })
      pdf.addImage(cv.toDataURL('image/png'), 'PNG', 0, 0, pdfW, pdfH)
  ...
  async function handlePNG() {
    ...
    try {
      ...
      cv.toBlob(blob => {
        ...
        a.href = URL.createObjectURL(blob); a.click(); toast.success('تم التحميل')
      }, 'image/png')
    } catch { toast.error('فشل التحميل') } finally { setLoading(false) }
  }
  ...
  const bg = theme === 'gradient' ? `linear-gradient(135deg,${colors.primary},${colors.secondary})`
    ...
    <div className="w-full h-full rounded-2xl p-4 flex flex-col justify-between overflow-hidden" style={{ background:bg, border, direction:'rtl' }}>
      <div className="flex items-start justify-between">
        <div className="rounded-xl p-2 flex-shrink-0" style={{ background: isDark?'rgba(255,255,255,0.12)':`${colors.primary}22` }}>
          <div className="w-7 h-7 grid grid-cols-3 gap-0.5">
            {Array.from({length:9}).map((_,i)=>(
              <div key={i} className="rounded-[2px]" style={{ background:[0,1,3,4,7,8].includes(i)?(theme==='gradient'?'rgba(255,255,255,0.9)':isDark?'#FFF':colors.primary):'transparent' }}/>
            ))}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {logoUrl ? <img src={logoUrl} alt="" className="w-10 h-10 rounded-xl object-cover flex-shrink-0"/> :
  ...
  const PreviewSquare = (
    <div className="w-full h-full rounded-2xl flex flex-col items-center overflow-hidden pt-1" style={{background:bg,border}}>
      ...
      <div className="flex flex-col items-center mt-4 gap-1">
        {logoUrl ? <img src={logoUrl} alt="" className="w-14 h-14 rounded-full object-cover ring-2" style={{ringColor:colors.primary}}/> :
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl"
            style={{background:`linear-gradient(135deg,${colors.primary},${colors.secondary})`}}>{ini}</div>}
  ...
  const PreviewPortrait = (
    <div className="w-full h-full rounded-2xl flex flex-col items-center overflow-hidden" style={{background:bg,border}}>
      ...
      <div className="flex flex-col items-center mt-5 gap-1">
        {logoUrl ? <img src={logoUrl} alt="" className="w-12 h-12 rounded-full object-cover"/> :
  ...
  const PreviewLarge = (
    <div className="w-full h-full rounded-2xl flex overflow-hidden" style={{background:bg,border}}>
      <div className="w-16 flex flex-col items-center justify-center flex-shrink-0" style={{background:`linear-gradient(180deg,${colors.primary},${colors.secondary})`}}>
        {logoUrl ? <img src={logoUrl} alt="" className="w-10 h-10 rounded-full object-cover"/> :
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold"
            style={{background:'rgba(255,255,255,0.25)'}}>{ini}</div>}
      </div>
  ...
  const BackPreview = (
    <div className="w-full h-full rounded-2xl flex flex-col items-center justify-center gap-2 overflow-hidden" style={{background:bg,border}}>
      <div className="p-2.5 rounded-xl bg-white shadow">
        <QRCodeCanvas value={qr} size={70} fgColor={colors.primary} bgColor="#FFFFFF" level="H"/>
      </div>
      <p className="text-[8px] px-3 text-center break-all" dir="ltr" style={{color:isDark?'rgba(255,255,255,0.6)':'#6B7280'}}>{qr}</p>
    </div>
  )

  const previewMap = { standard: PreviewStandard, square: PreviewSquare, portrait: PreviewPortrait, large: PreviewLarge }
  const FrontPreview = previewMap[sizeId]
  const ar = size.H / size.W  // aspect ratio
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-3xl p-6 space-y-4" style={{background:'var(--surface)',border:'1px solid var(--border)'}}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {SIZES.map(s=>(
                <button key={s.id} onClick={()=>setSizeId(s.id)}
                  className="flex items-center justify-between py-2 px-3 rounded-xl text-xs font-medium transition-all border"
          ...
          <div style={{aspectRatio:`${size.W}/${size.H}`}}>{FrontPreview}</div>
        ...
        <div className="rounded-3xl p-5 space-y-5" style={{background:'var(--surface)',border:'1px solid var(--border)'}}>
          <div>
            ...
            <div className="space-y-2">
              {[
                ...
                <input key={i} value={f.val} onChange={e=>f.set(e.target.value)} placeholder={f.ph} dir={f.dir as any}
          ...
          <div>
            ...
            <label className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer" style={{background:'var(--bg)',border:'1.5px dashed var(--border)'}}>
              ...
              {logoUrl ? (<><img src={logoUrl} alt="logo" className="w-8 h-8 rounded-lg object-contain"/><span className="text-xs font-medium" style={{color:'#4B9EFF'}}>تغيير الشعار</span></>) :
                (<><div className="w-8 h-8 rounded-lg flex items-center justify-center text-lg" style={{background:`${colors.primary}18`}}>🖼️</div><span className="text-xs" style={{color:'var(--text-muted)'}}>ارفع شعارك (PNG, JPG — حد 2MB)</span></>)}
            </label>
            {logoUrl&&<button onClick={()=>setLogoUrl(null)} className="text-xs mt-1 px-2" style={{color:'#EF4444'}}>حذف الشعار</button>}
          </div>
          ...
          <div className="space-y-2 pt-1">
            <button onClick={handlePDF} disabled={loading} className="btn-primary w-full text-sm disabled:opacity-50 flex items-center justify-center gap-2">
              {loading?'⏳':'📄'} {downloadPdfLabel}
            </button>
            <button onClick={handlePNG} disabled={loading} className="btn-secondary w-full text-sm disabled:opacity-50 flex items-center justify-center gap-2">
