...
export default function PrintDesigner({ title, subtitle, steps, downloadPdfLabel, downloadPngLabel, noteLabel, profileName, profileTitle, profilePhone, profileEmail }: Props) {
  ...
  async function drawFrontStandard(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    if (logoUrl) { const img=await loadImg(logoUrl); ctx.save(); ctx.beginPath(); ctx.roundRect(AX,AY,AS,AS,10); ctx.clip(); ctx.drawImage(img,AX,AY,AS,AS); ctx.restore() }
      ...
      ctx.fill(); ctx.fillStyle='#FFF'; ctx.font=`bold ${Math.round(AS*.4)}px Tahoma,Arial`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(n.charAt(0),AX+AS/2,AY+AS/2)
    }
  ...
  async function drawFrontSquare(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    ctx.save();ctx.direction='rtl';ctx.textAlign='center';ctx.textBaseline='top';ctx.fillStyle=txtC;ctx.font='bold 15px Tahoma,Arial';ctx.fillText(n,ox+W/2,AY+AS+14);ctx.fillStyle=subC;ctx.font='12px Tahoma,Arial';ctx.fillText(jobTitle||'المسمى الوظيفي',ox+W/2,AY+AS+32);ctx.restore()
    const divY=AY+AS+55;ctx.save();ctx.globalAlpha=.15;ctx.fillStyle=txtC;ctx.fillRect(ox+30,divY,W-60,1);ctx.restore()
    ctx.save();ctx.direction='ltr';ctx.textAlign='left';ctx.textBaseline='top';ctx.fillStyle=metC;ctx.font='11px Arial,sans-serif';let cy=divY+12;const lx=ox+30;if(phone){ctx.fillText('📞  '+phone,lx,cy);cy+=20}if(email){ctx.fillText('✉  '+email,lx,cy);cy+=20}ctx.font='10px Arial,sans-serif';ctx.fillText('🌐  '+qr,lx,cy);ctx.restore()
  }
  ...
  async function drawFrontPortrait(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    ctx.save();ctx.direction='rtl';ctx.textAlign='center';ctx.textBaseline='top';ctx.fillStyle=txtC;ctx.font='bold 14px Tahoma,Arial';ctx.fillText(n,ox+W/2,AY+AS+14);ctx.fillStyle=subC;ctx.font='11px Tahoma,Arial';ctx.fillText(jobTitle||'المسمى الوظيفي',ox+W/2,AY+AS+30);ctx.restore()
    const lineY=AY+AS+52;ctx.save();ctx.globalAlpha=.2;ctx.fillStyle=txtC;ctx.fillRect(ox+16,lineY,W-32,1);ctx.restore()
    ...
    ctx.save();ctx.direction='ltr';ctx.textAlign='left';ctx.textBaseline='top';ctx.fillStyle=metC;ctx.font='12px Arial,sans-serif';let cy=lineY+14;const lx=ox+20;if(phone){ctx.fillText('📞  '+phone,lx,cy);cy+=24}if(email){ctx.fillText('✉  '+email,lx,cy);cy+=24}ctx.font='10px Arial,sans-serif';ctx.fillText('🌐  '+qr,lx,cy);ctx.restore()
  }
  ...
  async function drawFrontLarge(ctx: CanvasRenderingContext2D, W: number, H: number, ox: number, oy: number) {
    ...
    const RX=ox+96
    ctx.save();ctx.direction='rtl';ctx.textAlign='right';ctx.textBaseline='top';ctx.fillStyle=txtC;ctx.font='bold 18px Tahoma,Arial';ctx.fillText(n,ox+W-16,oy+22);ctx.fillStyle=subC;ctx.font='13px Tahoma,Arial';ctx.fillText(jobTitle||'المسمى الوظيفي',ox+W-16,oy+46);ctx.restore()
  ...
  const AvatarEl = ({ size: sz, rounded }: { size: string; rounded: string }) => (
    logoUrl
    ...
    <div className="w-full h-full rounded-2xl p-4 flex flex-col justify-between overflow-hidden" style={{background:bg,border,direction:'rtl'}}>
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="text-right"><div className="font-bold text-sm" style={{color:txtC}}>{name||'اسمك هنا'}</div><div className="text-xs mt-0.5" style={{color:subC}}>{jobTitle||'المسمى الوظيفي'}</div></div>
          <AvatarEl size="w-10 h-10" rounded="rounded-xl"/>
        </div>
      </div>
  ...
  const PreviewSquare = (
    <div className="w-full h-full rounded-2xl flex flex-col items-center overflow-hidden pt-1" style={{background:bg,border}}>
      ...
      <div className="flex flex-col items-center mt-4 gap-1">
        <AvatarEl size="w-14 h-14" rounded="rounded-full"/>
  ...
  const PreviewPortrait = (
    <div className="w-full h-full rounded-2xl flex flex-col items-center overflow-hidden" style={{background:bg,border}}>
      ...
      <div className="flex flex-col items-center mt-5 gap-1">
        <AvatarEl size="w-12 h-12" rounded="rounded-full"/>
  ...
  const PreviewLarge = (
    <div className="w-full h-full rounded-2xl flex overflow-hidden" style={{background:bg,border}}>
      <div className="w-16 flex flex-col items-center justify-center flex-shrink-0" style={{background:`linear-gradient(180deg,${colors.primary},${colors.secondary})`}}>
        <AvatarEl size="w-10 h-10" rounded="rounded-full"/>
      </div>
      <div className="flex flex-col justify-between p-3 flex-1" dir="rtl">
        <div><div className="font-bold text-sm" style={{color:txtC}}>{name||'اسمك هنا'}</div><div className="text-xs mt-0.5" style={{color:subC}}>{jobTitle||'المسمى الوظيفي'}</div></div>
        ...
        <div className="rounded-3xl p-5 space-y-5" style={{background:'var(--surface)',border:'1px solid var(--border)'}}>
          <div>
            ...
            <div className="space-y-2">
              {([{val:name,set:setName,ph:'الاسم الكامل',dir:'rtl'},{val:jobTitle,set:setJobTitle,ph:'المسمى الوظيفي',dir:'rtl'},{val:phone,set:setPhone,ph:'رقم الهاتف',dir:'ltr'},{val:email,set:setEmail,ph:'البريد الإلكتروني',dir:'ltr'},{val:qrValue,set:setQrValue,ph:'رابط QR',dir:'ltr'}] as const).map((f,i)=>(
                <input key={i} value={f.val} onChange={e=>(f.set as any)(e.target.value)} placeholder={f.ph} dir={f.dir}
