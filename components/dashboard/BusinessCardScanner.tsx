  ...
  if (!isPro) {
    return (
      <div className="text-center py-10">
        ...
        <h3 className="text-lg font-bold mb-2">ميزة برو حصرية</h3>
        <p className="text-[var(--text-muted)] text-sm mb-6 max-w-xs mx-auto">
          ماسح بطاقات الأعمال بالذكاء الاصطناعي متاح فقط لمشتركي خطة برو
        </p>
        <button className="btn-primary">
          ترقَّ إلى برو — 15 ريال/شهر
        </button>
      </div>
    )
  }
    ...
    <div className="space-y-5">
      ...
      {!image ? (
        <div
          ...
          className="border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors hover:border-[#4B9EFF]"
          ...
          <button onClick={handleScan} disabled={scanning} className="btn-primary w-full disabled:opacity-60">
            {scanning ? (
              <span className="flex items-center justify-center gap-2">
                ...
                جاري تحليل البطاقة بالذكاء الاصطناعي...
              </span>
            ) : (
      ...
      {/* Results */}
      {result && (
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: '#4B9EFF40' }}>
          <div className="px-4 py-3 flex items-center gap-2"
            style={{ background: 'linear-gradient(135deg, #4B9EFF15, #8B5CF615)' }}>
          ...
          <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {[
              ...
              { key: 'email', label: 'البريد الإلكتروني', icon: '✉️' },
                ...
                <div key={key} className="px-4 py-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-medium text-sm truncate" dir={key === 'email' || key === 'website' || key === 'phone' ? 'ltr' : 'rtl'}>
                        {val}
                      </p>
                    </div>
                  </div>
                  <button onClick={() => copyField(val, label)}
                    className="flex-shrink-0 p-1.5 rounded-lg hover:bg-[var(--surface)] text-[var(--text-muted)] hover:text-[#4B9EFF] transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
