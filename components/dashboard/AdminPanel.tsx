...
export default function AdminPanel() {
  ...
  const updateUser = async (id: string, updates: Partial<AdminUser>) => {
    ...
    if (res.ok) { showToast("تم التحديث ✓"); fetchUsers(); }
  ...
  const sendNotification = async () => {
    ...
    setSending(false);
    if (res.ok) { showToast("تم الإرسال ✓"); setNotification({ title: "", body: "", target: "all", userId: "" }); }
            ...
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  ...
                  { label: "تسجيلات اليوم", value: stats.today_signups, icon: "🆕" },
                  { label: "مستخدمو Pro", value: stats.pro_users, icon: "⭐" },
                  { label: "محظورون", value: stats.banned_users, icon: "🚫" },
                ].map((kpi) => (
                  ...
                  {stats.daily_signups?.map((d, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
                        <div className="text-xs text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">{d.count}</div>
                        <div className="w-full rounded-t bg-indigo-500 hover:bg-indigo-400 transition-all cursor-default" style={{ height: `${Math.max(height, 4)}%` }} />
                        <div className="text-xs text-gray-500 -rotate-45 origin-top-right whitespace-nowrap">{d.date?.slice(5)}</div>
                      </div>
                    );
                  })}
                </div>
                    ...
                    return (
                      <div key={c.country} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 bg-gray-700 rounded-full h-2">
                          <div className="bg-indigo-500 h-2 rounded-full transition-all" style={{ width: `${(c.count / max) * 100}%` }} />
                        </div>
      ...
      {activeTab === "users" && (
        <div className="space-y-4">
          ...
          <div className="bg-gray-800/60 rounded-2xl border border-gray-700/50 overflow-hidden">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={u.id} className="border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <select value={u.plan || "free"} onChange={(e) => updateUser(u.id, { plan: e.target.value as Plan })}
                      ...
                      <td className="px-4 py-3">
                        <select value={u.status || "active"} onChange={(e) => updateUser(u.id, { status: e.target.value as UserStatus })}
                          className={`bg-gray-700 border border-gray-600 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 ${u.status === "banned" ? "text-red-400" : u.status === "suspended" ? "text-amber-400" : "text-emerald-400"}`}>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => deleteUser(u.id)} className="text-red-400 hover:text-red-300 text-xs px-2 py-1 rounded-lg hover:bg-red-500/10 transition-colors">
                          حذف
                        </button>
                      </td>
                    </tr>
            ...
            <div className="px-4 py-3 text-gray-500 text-xs border-t border-gray-700">{users.length} مستخدم</div>
          </div>
        </div>
      )}
            ...
            {plans.map((plan) => (
              <div key={plan.id} className="bg-gray-800/60 rounded-2xl p-6 border border-gray-700/50">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => setEditingPlan({ ...plan })} className="text-xs bg-indigo-600/20 text-indigo-400 px-3 py-1 rounded-lg hover:bg-indigo-600/40 transition-colors">
                    تعديل
                  </button>
                </div>
                <div className="text-3xl font-bold text-white mb-1">${plan.price_monthly}<span className="text-sm text-gray-400">/شهر</span></div>
                ...
                <div className="mt-3 space-y-1">
                  {plan.features?.map((f, i) => (
                    <div key={i} className="text-xs text-gray-400 flex items-center gap-1"><span className="text-emerald-500">✓</span> {f}</div>
                  ))}
                </div>
              </div>
            ))}
            ...
            <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setEditingPlan(null)}>
              <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md border border-gray-700" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={field.key}>
                      ...
                      <input type={field.type} value={(editingPlan as any)[field.key]}
                  ...
                  <div>
                    ...
                    <textarea rows={4} value={editingPlan.features?.join("\n")}
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={savePlan} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl py-2.5 text-sm font-medium transition-colors">حفظ التغييرات</button>
                  <button onClick={() => setEditingPlan(null)} className="flex-1 bg-gray-700 hover:bg-gray-600 text-white rounded-xl py-2.5 text-sm font-medium transition-colors">إلغاء</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-gray-800/60 rounded-2xl p-6 border border-gray-700/50 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[{ v: "all", l: "🌐 الكل" }, { v: "pro", l: "⭐ Pro" }, { v: "free", l: "🆓 Free" }, { v: "user", l: "👤 محدد" }].map((opt) => (
                  <button key={opt.v} onClick={() => setNotification({ ...notification, target: opt.v })}
            ...
            {notification.target === "user" && (
              <div>
                ...
                <input value={notification.userId} onChange={(e) => setNotification({ ...notification, userId: e.target.value })}
            ...
            <div>
              ...
              <input value={notification.title} onChange={(e) => setNotification({ ...notification, title: e.target.value })}
                placeholder="عنوان الإشعار..."
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">المحتوى</label>
              <textarea rows={4} value={notification.body} onChange={(e) => setNotification({ ...notification, body: e.target.value })}
