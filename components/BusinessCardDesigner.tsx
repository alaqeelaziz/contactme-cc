...
export default function BusinessCardDesigner() {
  ...
  // ── Canvas: draws FRONT (ox=0) + BACK (ox=W+GAP) on one canvas ────────────
  async function generateCanvas(): Promise<HTMLCanvasElement> {
    ...
    // Name & job (LTR — safe for mixed content)
    const TX = AX+AS+12
    ...
    ctx.beginPath(); ctx.roundRect(LMX-5,LMY-5,42,42,10)
    ctx.fillStyle=isDark?'rgba(255,255,255,0.12)':`${colors.primary}22`; ctx.fill()
    for(let i=0;i<9;i++){
      if(![0,1,3,4,7,8].includes(i)) continue
      ctx.beginPath(); ctx.roundRect(LMX+(i%3)*10.5, LMY+Math.floor(i/3)*10.5, 9, 9, 2)
    ...
    ctx.beginPath(); ctx.roundRect(qx-pad,qy-pad,qs+pad*2,qs+pad*2,12)
    ctx.fillStyle='#FFF'; ctx.fill()
    const qi=new Image(); await new Promise<void>(r=>{qi.onload=()=>r();qi.src=du})
    ctx.drawImage(qi,qx,qy,qs,qs)

    // URL — force LTR with save/restore
    ctx.save()
    ctx.direction='ltr'; ctx.textAlign='center'; ctx.textBaseline='top'
    ctx.fillStyle=metaColor; ctx.font=`9px Arial,sans-serif`
    ctx.fillText(qrValue, ox+W/2, qy+qs+pad+4)
    ctx.restore()
  ...
  async function handlePdf() {
    ...
    try {
      ...
      // Both sides side-by-side on one landscape page
      // Standard card 85.6×54 mm → both = 171.2×54 mm + gap
      const mmH = 54, mmW = 171.2 + 4   // 2 cards + 4mm gap
          ...
          <div className="w-8 h-8 grid grid-cols-3 gap-0.5">
            {Array.from({length:9}).map((_,i)=>(
              <div key={i} className="rounded-[2px]"
                style={{background:[0,1,3,4,7,8].includes(i)?(theme==='gradient'?'rgba(255,255,255,0.9)':isDark?'#FFF':colors.primary):'transparent'}}/>
            ))}
          </div>
        </div>
      </div>
  ...
  const BackFace = (
    <div className="w-full h-full rounded-2xl overflow-hidden flex flex-col items-center justify-center gap-3"
      ...
      <p className="text-[10px] px-6 text-center break-all w-full" dir="ltr" style={{color:t.metaText}}>{qrValue}</p>
    </div>
  )
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-4">
          ...
          {/* Download buttons — NO print button */}
          <div className="grid grid-cols-2 gap-2 pt-2">
            <button onClick={handlePdf} disabled={!!loading}
              className="py-3 rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2 disabled:opacity-60"
              style={{background:`linear-gradient(135deg,${colors.primary},${colors.secondary})`}}>
              {loading==='pdf'?<Spinner/>:'📄'} PDF
            </button>
            <button onClick={handlePng} disabled={!!loading}
              className="py-3 rounded-xl text-sm font-semibold border flex items-center justify-center gap-2 disabled:opacity-60"
              style={{borderColor:colors.primary,color:colors.primary}}>
              {loading==='png'?<Spinner/>:'🖼'} PNG
            </button>
          </div>
          <p className="text-center text-[11px] text-[var(--text-muted)]">
            💡 مجاني تماماً — الوجهان في ملف واحد
          </p>
        </div>
