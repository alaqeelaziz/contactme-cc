...
type Tab = 'profile' | 'links' | 'services' | 'card' | 'qr' | 'scanner' | 'analytics' | 'admin'

const ADMIN_ID = '4c33904a-2041-48de-a53d-907f5b532a18'

export default function DashboardClient({ initialProfile, initialLinks, initialServices, viewCount, profileUrl }: Props) {
  ...
  const isAdmin =
    ...
    profile.username    === 'aalaqeel03'

  const TABS: { id: Tab; label: string; icon: string }[] = [
    { id: 'profile',   label: t('tabs.profile'),   icon: '👤' },
    { id: 'links',     label: t('tabs.links'),      icon: '🔗' },
    ...(profile.account_type === 'company'
      ? [{ id: 'services' as Tab, label: t('tabs.services'), icon: '💼' }] : []),
    { id: 'card',      label: 'بطاقتي',             icon: '💳' },
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="card">
          ...
          {activeTab==='card'      && <BusinessCardTab profile={profile} profileUrl={profileUrl}/>}
