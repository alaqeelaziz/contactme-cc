...
export default async function HomePage({ params }: Props) {
  ...
  const heroContent: Record<string, { badge: string; title1: string; title2: string; description: string; ctaPrimary: string; ctaSecondary: string }> = {
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <section className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <Link href={`/${locale}/login`} className="btn-primary px-6 py-3 text-base">
            {hero.ctaPrimary}
          </Link>
          <Link href={`/${locale}/alaqeelaziz`} className="btn-secondary px-6 py-3 text-base">
            {hero.ctaSecondary}
          </Link>
        </div>
      </section>
